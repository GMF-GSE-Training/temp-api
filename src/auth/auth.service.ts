...
export class AuthService {
    constructor(
        ...
        @Inject('VERIFICATION_JWT_SERVICE') private readonly verificationJwtService: JwtService,
    ) {}
    ...
    async refreshTokens(refreshToken: string): Promise<AuthResponse> {
        try {
            const verifyRefreshToken = await this.refreshJwtService.verifyAsync(refreshToken);
    ...
    async profile(user: CurrentUserRequest): Promise<AuthResponse> {
        ...
        if (findUser.role.name === 'user' && findUser.participant) {
            ...
            const isComplete = requiredFields.every(field => 
                findUser.participant[field] !== null && findUser.participant[field] !== undefined
            );
    ...
    async resetPassword(request: ResetPassword): Promise<string> {
        ...
        try {
            ...
            if(!user.passwordResetToken || user.passwordResetToken !== request.token) {
                throw new HttpException('Token ini tidak valid atau sudah digunakan', 400);
            }
