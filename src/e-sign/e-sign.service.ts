...
import { CreateESign, ESignResponse, SignatureType, UpdateESign } from "src/model/e-sign.model";
...
export class ESignService {
    ...
    async createESign(request: CreateESign): Promise<string> {
        const createRequest = this.validationService.validate(ESignValidation.CREATE, request);
        ...
        if (createRequest.status === true) {
            const existingActiveSignature = await this.prismaService.signature.findFirst({
                where: {
                    status: true,
    ...
    async updateESign(eSignId: string, request: UpdateESign): Promise<string> {
        const updateRequest = this.validationService.validate(ESignValidation.UPDATE, request);
        ...
        if(updateRequest.status) {
            if (updateRequest.status === true) {
                const existingActiveSignature = await this.prismaService.signature.count({
                    where: {
                        status: true,
    ...
    async listESign(request: ListRequest, user: CurrentUserRequest): Promise<{ data: ESignResponse[], actions: ActionAccessRights, paging: Paging }> {
        let whereClause: any = {};
