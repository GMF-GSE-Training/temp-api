...
export class CertificateService {
    ...
    async createCertificate(cotId: string, participantId: string, request: CreateCertificate): Promise<any> {
        const createCertificateRequest = this.validationService.validate(CertificateValidation.CREATE, request);
        ...
        if(!cot) {
            throw new HttpException('Gagal membaut sertifikat. COT tidak ditemukan', 404);
        }
        ...
        if(!eSign) {
            throw new HttpException('Gagal membaut sertifikat. Tidak ada Esign yang aktif', 404);
        }
