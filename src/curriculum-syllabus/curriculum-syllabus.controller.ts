...
export class CurriculumSyllabusController {
    ...
    @Roles('super admin')
    @UseGuards(AuthGuard, RoleGuard)
    async update(@Param('capabilityId', ParseUUIDPipe) capabilityId: string, @Body() request: UpdateCurriculumSyllabus): Promise<WebResponse<string>> {
        const result = await this.curriculumSyllabusService.updateCurriculumSyllabus(capabilityId, request);
        return buildResponse(HttpStatus.OK, result);
    }
}